...
class NST:
    ...
    def __init__(self, style_image, content_image, alpha=1e4, beta=1, var=10):
        """
        Initializes the NST class instance with Variational Cost weights
        """
    ...
    def variational_cost(generated_image):
        """
        Calculates the total variational regularization cost for smoothing
        """
        ...
        # Sum of squared spatial pixel delta gaps
        h_diff = tf.square(
        ...
        return tf.reduce_sum(h_diff) + tf.reduce_sum(v_diff)

    def total_cost(self, generated_image):
        """
        Calculates the weighted total loss including variational smoothing
        """
        if (not isinstance(generated_image, tf.Tensor) and
                not isinstance(generated_image, tf.Variable)):
            raise TypeError(
    ...
    def generate_image(self, iterations=1000, step=None, lr=0.01,
        ...
        return final_image.numpy(), best_cost
