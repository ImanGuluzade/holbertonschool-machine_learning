...
class NST:
    ...
    def content_cost(self, content_output):
        ...
        loss = tf.reduce_sum(tf.square(content_output - self.content_feature))
    ...
    def total_cost(self, generated_image):
        ...
        # Scale up and preprocess the generated image for feature extraction
        preprocessed_gen = tf.keras.applications.vgg19.preprocess_input(
            generated_image * 255.0
        )

        gen_outputs = self.model(preprocessed_gen)
        gen_style_outputs = gen_outputs[:-1]
        gen_content_output = gen_outputs[-1]

        # Calculate isolated sub-costs
        j_content = self.content_cost(gen_content_output)
        j_style = self.style_cost(gen_style_outputs)

        # Apply weights to find composite total cost J
