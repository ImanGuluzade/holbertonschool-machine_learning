...
class Yolo:
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        ...
        Parameters:
            ...
            box_scores: numpy.ndarray containing scores for each box.
            file_name: string representing target display window name.
        """
        # Create a copy so we do not modify the original raw image array
        disp_img = image.copy()

        for i in range(len(boxes)):
            # Cast boundary coordinate elements safely to integer pixels
            x1 = int(boxes[i][0])
            y1 = int(boxes[i][1])
            x2 = int(boxes[i][2])
            y2 = int(boxes[i][3])

            # Draw the boundary bounding box (Blue line: BGR -> (255, 0, 0))
            cv2.rectangle(disp_img, (x1, y1), (x2, y2), (255, 0, 0), 2)

            # Generate labels text format (e.g., 'dog 1.00')
            cls_name = self.class_names[box_classes[i]]
            score = box_scores[i]
            label = "{} {:.2f}".format(cls_name, score)

            # Position text precisely 5 pixels above top-left corner
            text_pos = (x1, y1 - 5)

            # Write label using Red text (BGR -> (0, 0, 255))
            cv2.putText(
                disp_img, label, text_pos, cv2.FONT_HERSHEY_SIMPLEX,
                0.5, (0, 0, 255), 1, cv2.LINE_AA
            )

        # Handle window render and wait interaction
        cv2.imshow(file_name, disp_img)
        key = cv2.waitKey(0)

        # If 's' key is pressed, write down output to detections directory
        if key == ord('s'):
