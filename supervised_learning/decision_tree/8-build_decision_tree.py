...
class Decision_Tree():
    ...
    def possible_thresholds(self, node, feature):
        """Computes all possible mid-point thresholds for a feature"""
        values = np.unique((self.explanatory[:, feature])[node.sub_population])
        return (values[1:] + values[:-1]) / 2

    def Gini_split_criterion_one_feature(self, node, feature):
        """Computes the best threshold and Gini average for a feature"""
        ...
        # Broadcasting shapes for comparison
        # (n, 1, 1) vs (1, t, 1) vs (1, 1, c)
        targets_3D = Y[:, np.newaxis, np.newaxis]
        classes_3D = classes[np.newaxis, np.newaxis, :]
        feat_3D = X[:, np.newaxis, np.newaxis]
        thresh_3D = thresholds[np.newaxis, :, np.newaxis]

        # i-th individual class k AND feature > threshold j
        Left_F = (targets_3D == classes_3D) & (feat_3D > thresh_3D)
        Right_F = (targets_3D == classes_3D) & (feat_3D <= thresh_3D)

        count_l = np.sum(Left_F, axis=0)  # (t, c)
        count_r = np.sum(Right_F, axis=0)  # (t, c)

        n_l = np.sum(count_l, axis=1)  # (t,)
        n_r = np.sum(count_r, axis=1)  # (t,)

        # Weighted Gini computation
        with np.errstate(divide='ignore', invalid='ignore'):
            gini_l = 1 - np.sum((count_l / n_l[:, np.newaxis])**2, axis=1)
            gini_r = 1 - np.sum((count_r / n_r[:, np.newaxis])**2, axis=1)
            # Fill NaNs where population was zero
    ...
    def Gini_split_criterion(self, node):
        """Chooses the best feature and threshold using Gini Impurity"""
    ...
    def get_leaf_child(self, node, sub_population):
        """Calculates majority class for a leaf"""
