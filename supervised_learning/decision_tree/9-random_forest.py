...
class Random_Forest():
    ...
    def predict(self, explanatory):
        """
        Predicts the class for each individual in the explanatory data
        by taking the mode of predictions from all trees in the forest
        """
        # Generate predictions for each tree in the forest
        # Each tree.predict(explanatory) returns an array of size (n_examples,)
        # We collect these into a list to create a 2D array
        all_preds = np.array([p(explanatory) for p in self.numpy_preds])

        # Calculate the mode (most frequent) prediction for each example
        # axis=0 corresponds to the dimension of the different trees
        # We use a bincount approach along the column axis for speed
        def get_mode(column):
            return np.bincount(column.astype(int)).argmax()

        return np.apply_along_axis(get_mode, 0, all_preds)
