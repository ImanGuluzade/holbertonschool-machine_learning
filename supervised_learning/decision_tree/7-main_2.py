...
def circle_of_clouds(n_clouds, n_objects_by_cloud, radius=1, sigma=None, seed=0, angle=0):
    """ Generates the circle of clouds dataset """
...
def np_extrema(arr):
    """ Helper to get min/max for plot bounds """
    return np.min(arr), np.max(arr)

def visualize_bassins(ax, model, x_min, x_max, y_min, y_max, cmap):
    """ Colors the background based on model predictions """
...
def visualize_training_dataset_2D(ax, model, cmap):
    """ Plots the actual training data points """
    ax.scatter(model.explanatory[:, 0], model.explanatory[:, 1], c=model.target, cmap=cmap, edgecolors='k')

def visualize_model_2D(model, cmap=plt.cm.Set1):
    """ Creates the final side-by-side visualization """
    assert model.explanatory.shape[1] == 2, "Not a 2D example"
    ...
    visual_training_dataset_2D(axes[0], model, cmap)
    
    axes[1].set_title("Decision Bassins (Predictions)")
    visual_bassins(axes[1], model, x_min, x_max, y_min, y_max, cmap)
    
    plt.savefig("bassins2.png")
    print("Visualization saved as bassins2.png")
    plt.show()
